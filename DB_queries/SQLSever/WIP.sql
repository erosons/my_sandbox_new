SELECT distinct
    ...
    ca.operating_company as operating_company,
    q.quote_opportunity_type as opportunity_type,
    sub_base.customerName,
...
left join (
            select distinct 
            account_id,account_name as operating_company
            from mp2IntelMsAccess.tbExtractDataOperatingCompany 
                ) ca on ca.account_id =q.operating_company_id 
left join (
        SELECT 
            quote_id, 
...
left join (
    ...
    ) sub_q on sub_q.quote_id = q.quote_id
WHERE 1=1
    and q.quote_create_dttm >= '2020-01-01'
    and q.quote_broker not like '@%'
    and q.quote_title not like'%add%'
    and ca.operating_company not in ('Steel City Energy','MP2 ENERGY TEXAS LLC 2 (LSE)','MP2 Energy NE, LLC','X_MP2 Energy IL, LLC') 
    --and q.quote_id='57710'
GROUP BY
    q.quote_number,
    q.quote_id,
    ca.operating_company,
    q.quote_opportunity_type,
    sub_base.customerName,
    --q.quote_account,
    q.quote_sales_person,
